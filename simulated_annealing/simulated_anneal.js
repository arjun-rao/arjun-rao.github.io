...
function probFunc(neighborSolution, currentSolution, temp) {
	var prob = Math.exp(-(energy(neighborSolution)-energy(currentSolution))/temp);
	temps[temp] = prob;
	console.log(temp, prob, Math.random() < prob);
...
function selectCoolingFunc(temp, progress) {
	switch (dropDown.options[dropDown.selectedIndex].text) {
		...
		case "Exponential":
			return exponential(temp, progress);
	}
}
...
function exponential(temp, progress) {
	return (temp - 0.9 * (progress*100));
}
...
function plotSolution() {
	...
	var trace1 = {
	  ...
	  var data = [trace1];
	  var layout = {
		title: 'Temp vs Acceptance Probability',
		xaxis: {
		  title: 'Temperature',
		  showgrid: false,
		  zeroline: false
