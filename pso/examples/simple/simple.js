(function () {
	...
	function drawPopulation() {
		...
		con2d.lineWidth = 1;
		con2d.strokeStyle = '#F04'; // Red line denotes position of each particle
		...
		// Uncomment to get the best position of each particle 
		// con2d.lineWidth = 1.2;
		// con2d.strokeStyle = '#1FA'; // Green Line
	...
	function addResultToList(result) {
		...
		document.getElementById("run_outputs").appendChild(node); 
		results.push(result);
		console.log(avg(results));
		document.getElementById("runs_avg").innerHTML = " " + avg(results);
	}
	...
	function loop() {
		if (running) {
			...
			if (iteration < iterationNMax) {
				...
				running = false;
				addResultToList(optimizer.getBestFitness())
			}
		}
	}
